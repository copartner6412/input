...
//  - Twitter 9/10 words
//  - Google 9/10 words
func Passphrase(minWords, maxWords uint, separator string, capitalize bool, number bool, wordList []string) (string, error) {
	...
	random1, err := rand.Int(rand.Reader, big.NewInt(int64(maxWords - minWords + 1)))
	if err != nil {
	...
	for i := 0; i < wordNumber; i++ {
		...
		random2, err := rand.Int(rand.Reader, big.NewInt(int64(len(wordList))))
		if err != nil {
	...
	if number {
		...
		random3, err := rand.Int(rand.Reader, big.NewInt(int64(len(passphraseWords))))
		if err != nil {
		...
		random4, err := rand.Int(rand.Reader, big.NewInt(10))
		if err != nil {
